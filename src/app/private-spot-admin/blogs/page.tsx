...
export default function BlogsPage() {
  const { currentUser, privateSpotAdminData, loading: authLoading } = useAuth();
  const router = useRouter();
  const [blogs, setBlogs] = useState<Blog[]>([]);
  const [loading, setLoading] = useState(true);
  
  // Redirect if not authenticated or not a private spot admin
  useEffect(() => {
    if (!authLoading && (!currentUser || !privateSpotAdminData)) {
      router.push('/login');
    }
  }, [currentUser, privateSpotAdminData, authLoading, router]);
  ...
  useEffect(() => {
    if (currentUser && privateSpotAdminData) {
      fetchBlogs();
    }
  ...
  const fetchBlogs = async (): Promise<void> => {
    try {
      ...
      // For private spot admin, fetch all blogs from their business
      const blogsQuery = query(
        blogsRef,
        where('businessId', '==', privateSpotAdminData?.id || privateSpotAdminData?.uid)
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const blogData: any = {
        ...
        barangay: privateSpotAdminData?.barangay || currentUser.barangay || '',
        businessId: privateSpotAdminData?.id || currentUser.uid,
        businessName: spotName,
        author: spotName, // Using private spot name as author
        authorName: spotName, // Using private spot name as author display name
        authorType: 'private_spot',
