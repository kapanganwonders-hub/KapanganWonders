...
export default function ReportsPage() {
  ...
  // period state: weekly | monthly | quarterly | yearly | all
  const [period, setPeriod] = useState<'weekly' | 'monthly' | 'quarterly' | 'yearly' | 'all'>('monthly');

  // helper: compute start date for selected period
  ...
  // visits filtered by selected period
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      // Fetch private spot owner info
      const privateRef = doc(db, 'privateSpotOwners', user.uid);
      const privateSnap = await getDoc(privateRef);

      if (privateSnap.exists()) {
        const ownerData = privateSnap.data();
        const ownerSpot = ownerData?.privateSpotName; // dedicated spot
        setPlaceName(ownerSpot);

        // Listen for completed visits that include this spot
          ...
          const data = snapshot.docs
            ...
            .filter(
              (v: any) =>
                v.status === 'Completed' &&
                v.spots?.some(
                  (s: string) => s.toLowerCase() === ownerSpot.toLowerCase()
                )
            );
          setVisits(data);
          setLoading(false);
    ...
    <div className="p-8">
      ...
      {periodFiltered.length === 0 ? (
        <p className="text-gray-500 text-center mt-10">
          No completed visits found for your private spot in the selected period ({period === 'all' ? 'All' : period}).
        </p>
      ) : (
