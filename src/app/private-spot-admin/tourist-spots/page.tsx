...
export default function TouristSpotsPage() {
  ...
  const fetchSpots = async () => {
    try {
      setLoading(true);
      if (!privateSpotAdminData || !currentUser?.uid) {
        console.log('fetchSpots: Missing privateSpotAdminData or currentUser.uid', { 
          hasPrivateSpotAdminData: !!privateSpotAdminData, 
          hasCurrentUserUid: !!currentUser?.uid 
        });
        return;
      }

      const spotsRef = collection(db, 'touristSpots');
      // Filter spots by the current admin's UID and business name
      const spotsQuery = query(
        spotsRef, 
        where('createdBy', '==', currentUser.uid),
        where('businessId', '==', privateSpotAdminData.uid || privateSpotAdminData.id)
      );
      
      console.log('fetchSpots: Querying spots for admin:', {
        uid: currentUser.uid,
        businessId: privateSpotAdminData.uid || privateSpotAdminData.id,
        businessName: privateSpotAdminData.businessName || privateSpotAdminData.privateSpotName
      });
      
      const snapshot = await getDocs(spotsQuery);
      
      console.log('fetchSpots: Query result -', snapshot.docs.length, 'spots found');
      if (snapshot.empty) {
        console.log('fetchSpots: No spots found for this admin.');
      }
      ...
      console.log('Setting spots:', spotsData);
      setSpots(spotsData);
    } catch (err) {
      console.error('Error fetching tourist spots:', err);
      setAlertState({
        variant: 'destructive',
        message: 'Failed to load tourist spots. Please try again.'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleAddSpot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('handleAddSpot: Barangay name:', barangayName);

      console.log('handleAddSpot: Uploading image...');
      ...
      const spotData = {
        ...
        createdBy: currentUser.uid, // Add the creator's UID
        businessId: privateSpotAdminData.uid || privateSpotAdminData.id, // Add business ID
        businessName: privateSpotAdminData.businessName || privateSpotAdminData.privateSpotName || privateSpotAdminData.displayName, // Add business name
        createdAt: new Date(),
        updatedAt: new Date()
      };

      console.log('handleAddSpot: Creating spot with data:', spotData);
      const docRef = await addDoc(collection(db, 'touristSpots'), spotData);
